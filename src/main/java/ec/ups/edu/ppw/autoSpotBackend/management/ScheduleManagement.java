...
public class ScheduleManagement {
    ...
    public void addSchedule(Schedule shedule) throws Exception {
        ...
        if(shedule.getStatus().compareTo("R") == 0) throw  new Exception("NO SE PUEDE REGISTRAR MAS DIAS REGULARES");
        if(shedule.getStatus().compareTo("E") != 0 || shedule.getStatus().compareTo("NW") != 0)  throw  new Exception("NO CUMPLE CON LOS REQUISITOS");
    ...
    public void removeSchedule(int id_schedule) throws Exception {
        Schedule schedule = validatorForSchedule(id_schedule);
        if(schedule.getStatus().compareTo("R") == 0) throw  new Exception("NO SE PUEDE ELIMINAR UN DIA REGULAR");
